	...
	try {
		...
		if (await featureCategoryRepository.getFeatureCategoryByName(featureCategory.name)) {
			...
			try {
				...
				throw new Error(`Category name ${featureCategory.name} not found in wikipedia`)
			}
		}
	} catch (err) {
		throw err
...
async function get (featureCategory) {
	try {
        featureCategory.name = featureCategory.name.toLowerCase()
		const retrievedCategory = await featureCategoryRepository.getFeatureCategoryByName(featureCategory.name)
...
async function getAll() {
	try {
		return await featureCategoryRepository.getFeatureCategories()
	} catch (err) {
		throw err
	}
}
